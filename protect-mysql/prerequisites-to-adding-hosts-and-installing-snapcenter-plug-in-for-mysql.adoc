...
summary: Prima di aggiungere un host e installare i pacchetti plug-in, è necessario soddisfare tutti i requisiti.  Il plug-in SnapCenter per MySQL è disponibile sia negli ambienti Windows che Linux. 
---
...
[role="lead"]
Prima di aggiungere un host e installare i pacchetti plug-in, è necessario soddisfare tutti i requisiti.  Il plug-in SnapCenter per MySQL è disponibile sia negli ambienti Windows che Linux.
...
* Per Windows, il servizio Creator del plug-in dovrebbe essere eseguito utilizzando l'utente Windows "`LocalSystem`", che è il comportamento predefinito quando il plug-in per MySQL è installato come amministratore di dominio.
* Quando si installa un plug-in su un host Windows, se si specifica una credenziale non integrata o se l'utente appartiene a un utente di un gruppo di lavoro locale, è necessario disabilitare UAC sull'host.  Il plug-in SnapCenter per Microsoft Windows verrà distribuito per impostazione predefinita con il plug-in MySQL sugli host Windows.
* SnapCenter Server dovrebbe avere accesso alla porta 8145 o personalizzata del plug-in per l'host MySQL.
* Per MySQL 5.7, binlog deve essere specificato nel file di configurazione mysql (my.cnf o mysql-server.cnf).
...
+
https://imt.netapp.com/matrix/imt.jsp?components=121071;&solution=1259&isHWU&src=IMT["Strumento matrice di interoperabilità NetApp"]
...
* Per i database MySQL in esecuzione su un host Linux, durante l'installazione del plug-in per MySQL, viene installato automaticamente il plug-in SnapCenter per UNIX.
