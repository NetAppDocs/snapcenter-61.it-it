...
summary: È possibile utilizzare un plug-in SnapCenter con la tecnologia ONTAP SnapMirror per creare copie mirror di set di backup su un altro volume e con la tecnologia ONTAP SnapVault per eseguire la replicazione del backup da disco a disco per la conformità agli standard e altri scopi correlati alla governance.  Prima di eseguire queste attività, è necessario configurare una relazione di protezione dei dati tra i volumi di origine e di destinazione e inizializzare la relazione. 
---
= Preparare i sistemi di archiviazione per la replica SnapMirror e SnapVault
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
