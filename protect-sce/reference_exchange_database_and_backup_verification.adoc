...
---
= Verifica del database di Exchange e del backup
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Lo strumento Microsoft Exchange Eseutil è un'utilità della riga di comando inclusa nel server Exchange.  L'utilità consente di eseguire controlli di coerenza per verificare l'integrità dei database e dei backup di Exchange.

|===


| *Procedura consigliata:* non è necessario eseguire controlli di coerenza sui database che fanno parte di una configurazione DAG (Database Availability Group) con almeno due repliche. 
|===
